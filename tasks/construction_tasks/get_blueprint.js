...
bot.on('spawn', async () => {
  ...
  const taskFilePath =
    '/Users/isadorawhite/izzy_mindcraft/mindcraft/tasks/construction_tasks/custom/flower_three_agents.json';
  const task_name = 'flower_three_agents';

  setTimeout(async () => {
    let task_blueprint = await worldToBlueprint(
      startCoord,
      yOffset,
      xOffset,
      zOffset,
    ...
    fs.writeFileSync(
      taskFilePath,
      JSON.stringify(task_collection, null, 2),
      (err) => {
        if (err) {
