...
import { containsCommand } from './commands/const.js';
import {
  commandExists,
  executeCommand,
  truncCommandMessage,
...
export class Agent {
  async start(
    ...
    this.bot.once('spawn', async () => {
      try {
        ...
        void this._setupEventHandlers(save_data, init_message);
        this.startEvents();
        ...
        this.vision_interpreter = new VisionInterpreter(
          this,
  ...
  async _setupEventHandlers(save_data, init_message) {
    ...
    const respondFunc = async (username, message) => {
      ...
      try {
        ...
        if (convoManager.isOtherAgent(username)) {
          ...
          void this.handleMessage(username, translation);
        }
      } catch (error) {
    ...
    if (save_data?.self_prompt) {
      if (init_message) {
        void this.history.add('system', init_message);
      }
    ...
    if (save_data?.last_sender) {
      ...
      if (convoManager.otherAgentInGame(this.last_sender)) {
        ...
        void convoManager.receiveFromBot(this.last_sender, msg_package);
      }
    } else if (init_message) {
      void this.handleMessage('system', init_message, 2);
    } else {
  ...
  checkAllPlayersPresent() {
    ...
    const missingPlayers = this.task.agent_names.filter(
      (name) => !this.bot.players[name]
    );
  ...
  async handleMessage(source, message, max_responses = null) {
    ...
    if (!self_prompt && !from_other_bot) {
      ...
      if (user_command_name) {
        if (!commandExists(user_command_name)) {
          void this.routeResponse(
            source,
            `Command '${user_command_name}' does not exist.`
          );
          return false;
        }
        void this.routeResponse(
        ...
        if (user_command_name === '!newAction') {
          ...
          void this.history.add(source, message);
        }
        let execute_res = await executeCommand(this, message);
        if (execute_res) void this.routeResponse(source, execute_res);
        return true;
      }
    }
    ...
    for (let i = 0; i < max_responses; i++) {
      ...
      if (command_name) {
        ...
        void this.history.add(this.name, res);
        ...
        if (settings.verbose_commands) {
          void this.routeResponse(source, res);
        } else {
          ...
          void this.routeResponse(source, chat_message);
        }
        ...
        if (execute_res) void this.history.add('system', execute_res);
        else break;
      } else {
  ...
  async routeResponse(to_player, message) {
    ...
    if (
      convoManager.isOtherAgent(to_player) &&
      convoManager.inConversation(to_player)
    ) {
  ...
  startEvents() {
    ...
    this.bot.on('death', () => {
      void this.actions.cancelResume();
    ...
    if (this.task.data) {
      ...
      if (res) {
        await this.history.add(
          'system',
          `Task ended with score : ${res.score}`
        );
