...
export class SkillLibrary {
  ...
  async initSkillLibrary() {
    ...
    for (const skillName of this.always_show_skills) {
      this.always_show_skills_docs[skillName] = this.skill_docs.find((doc) =>
        doc.includes(skillName)
      );
    }
  }
  ...
  async getRelevantSkillDocs(message, select_num) {
    ...
    if (select_num === -1) {
      skill_doc_similarities = Object.keys(this.skill_docs_embeddings).map(
        (doc_key) => ({
          doc_key,
          similarity_score: 0,
        })
