...
class ConversationManager {
  ...
  sendToBot(send_to, message, start = false, open_chat = true) {
    if (!this.isOtherAgent(send_to)) {
      console.warn(
        `${agent.name} tried to send bot message to non-bot ${send_to}`
      );
      return;
    }
    const convo = this._getConvo(send_to);
    ...
    agent.serverProxy.getSocket().emit('chat-message', send_to, json);
  }
...
async function _scheduleProcessInMessage(sender, received, convo) {
  ...
  if (!agent.isIdle() && otherAgentBusy) {
    ...
    if (canTalkOver) {
      ...
      let shouldRespond = await agent.prompter.promptShouldRespondToBot(
        received.message
      );
      console.log(
        `${agent.name} decided to ${shouldRespond ? 'respond' : 'not respond'} to ${sender}`
      );
...
function _handleFullInMessage(sender, received) {
  console.log(
    `${agent.name} responding to "${received.message}" from ${sender}`
  );
