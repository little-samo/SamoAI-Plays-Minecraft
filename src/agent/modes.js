...
const modes_list = [
  {
    ...
    update: async function (agent) {
      ...
      if (blockAbove.name === 'water' || blockAbove.name === 'flowing_water') {
        ...
        void say(agent, "I'm on fire!"); // TODO: gets stuck in lava
        void execute(this, agent, async () => {
          let nearestWater = world.getNearestBlock(bot, 'water', 20);
          if (nearestWater) {
            const pos = nearestWater.position;
            await skills.goToPosition(bot, pos.x, pos.y, pos.z, 0.2);
            void say(agent, "Ahhhh that's better!");
          } else {
  ...
  },
  {
    ...
    update: async function (agent) {
      ...
      if (enemy && (await world.isClearPath(agent.bot, enemy))) {
        void say(agent, `Aaa! A ${enemy.name.replace('_', ' ')}!`);
        void execute(this, agent, async () => {
  ...
  },
  {
    ...
    update: async function (agent) {
      ...
      if (enemy && (await world.isClearPath(agent.bot, enemy))) {
        void say(agent, `Fighting ${enemy.name}!`);
        void execute(this, agent, async () => {
    ...
    update: async function (agent) {
      ...
      if (
        ...
        if (Date.now() - this.noticed_at > this.wait * 1000) {
          void say(agent, `Picking up item!`);
          this.prev_item = item;
          void execute(this, agent, async () => {
