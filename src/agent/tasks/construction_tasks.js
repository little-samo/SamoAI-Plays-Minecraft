    ...
    for (let zOffset = 0; zOffset < placement.length; zOffset++) {
      ...
      for (let xOffset = 0; xOffset < row.length; xOffset++) {
        ...
        try {
          ...
          const actualBlockName = blockAtLocation
            ? bot.registry.blocks[blockAtLocation.type].name
            : 'air';
...
export function proceduralGeneration(
  ...
  const matrix = Array.from({ length: p }, () =>
    Array.from({ length: m }, () => Array(n).fill('air'))
  );
  ...
  function validateAndBuildBorder(
    matrix,
    newX,
    newY,
    newZ,
    ...
    if (Math.random() < 0.8) {
      ...
      for (let dx = 0; dx <= windowX; dx++) {
        for (let dy = 0; dy <= windowY; dy++) {
          ...
          if (
            isInBounds(backZ, centerX + dx, centerY + dy) &&
    ...
    if (Math.random() < 0.8) {
      ...
      for (let dz = 0; dz <= windowZ; dz++) {
        for (let dy = 0; dy <= windowY; dy++) {
          ...
          if (
            isInBounds(centerZ + dz, leftX, centerY + dy) &&
    ...
    if (Math.random() < 0.8) {
      ...
      for (let dx = 0; dx <= windowX; dx++) {
        for (let dz = 0; dz <= windowZ; dz++) {
          ...
          if (
            isInBounds(centerZ + dz, centerX + dx, topY) &&
  ...
  while (placedRooms < rooms) {
    ...
    for (let attempt = 0; attempt < 150; attempt++) {
      const material =
        roomMaterials[Math.floor(Math.random() * roomMaterials.length)];

      // dimensions of room
      const newLength = Math.max(
        minRoomLength,
        Math.floor(Math.random() * roomVariance) + minRoomLength
      );
      const newWidth = Math.max(
        minRoomWidth,
        Math.floor(Math.random() * roomVariance) + minRoomWidth
      );
      const newDepth = Math.max(
        minRoomDepth,
        Math.floor(Math.random() * Math.floor(roomVariance / 2)) + minRoomDepth
      );
      let newX, newY, newZ;
...
async function getBlockName(bot, coordinate) {
  const blockAtLocation = bot.blockAt(
    new Vec3(coordinate.x, coordinate.y, coordinate.z)
  );
 ...
 */
export async function worldToBlueprint(
  startCoord,
  y_amount,
  x_amount,
  z_amount,
...
export function blueprintToTask(blueprint_data, num_agents) {
  ...
  for (const key of Object.keys(blueprint_data.materials)) {
    initialInventory[JSON.stringify(give_agent)][key] =
      blueprint_data.materials[key];
  ...
  const task = {
    ...
    conversation:
      "Let's share materials and make a structure with the blueprint",
