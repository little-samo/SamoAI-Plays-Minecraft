...
import { getCommandDocs } from '../agent/commands/docs.js';
...
export class Prompter {
  constructor(agent, fp) {
    ...
    let default_profile = JSON.parse(
  ...
  _selectAPI(profile) {
    ...
    if (!profile.api) {
      if (profile.model.includes('openrouter/'))
      ...
      else if (profile.model.includes('vllm/')) profile.api = 'vllm';
      else if (
      ...
      else if (profile.model.includes('replicate/')) profile.api = 'replicate';
      else if (
        ...
        profile.api = 'groq';
      else if (profile.model.includes('glhf/')) profile.api = 'glhf';
      else if (profile.model.includes('hyperbolic/'))
        profile.api = 'hyperbolic';
      else if (profile.model.includes('novita/')) profile.api = 'novita';
      ...
      else throw new Error('Unknown model:', profile.model);
    }
    return profile;
  }
  _createModel(profile) {
    ...
    if (profile.api === 'google')
      ...
      model = new GLHF(
        profile.model.replace('glhf/', ''),
        profile.url,
        profile.params
      );
    else if (profile.api === 'hyperbolic')
      model = new Hyperbolic(
        profile.model.replace('hyperbolic/', ''),
        profile.url,
        profile.params
      );
      ...
      model = new VLLM(
        profile.model.replace('vllm/', ''),
        profile.url,
        profile.params
      );
    else throw new Error('Unknown API:', profile.api);
    return model;
  }
  getName() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      try {
        ...
        if (typeof generation !== 'string') {
          console.error(
            'Error: Generated response is not a string',
            generation
          );
        ...
        console.error(
          'Error during message generation or file writing:',
          error
        );
        continue;
      }
      ...
      if (current_msg_time !== this.most_recent_msg_time) {
        console.warn(
          `${this.agent.name} received new message while generating, discarding old response.`
        );
        return '';
      }
  ...
  async _saveToFile(logFile, logEntry) {
    ...
    if (task_id == null) {
      ...
      logDir = path.join(
        __dirname,
        `../../bots/${this.agent.name}/logs/${task_id}`
      );
    }
